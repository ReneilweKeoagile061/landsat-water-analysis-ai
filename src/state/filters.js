export function filterPoints(points, filters) {
  return points.filter((feature) => {
    ...
    if (filters.minPotential > 0 && Number(props.high_prob) < filters.minPotential) return false;
    return true;
  });
}
...
export function readFiltersFromDom() {
  return {
    ...
    minPotential: Number(document.getElementById("potentialInput")?.value ?? 0) / 100,
  };
}
